...
public class ContratosClientesFindByUsuariosWebAction extends WebAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public void execute(WebController servlet, HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
		try {
			...
			// execute the finder
			ContratosClientes dto[] = dao.findByUsuarios(idUsuario);
		
			// store the results
			request.setAttribute( "result", dto );
		
			// forward request on to the appropriate JSP page to display the results
			forward( servlet, request, response, "/ContratosClientesFindByUsuariosResult.jsp" );
		}
		catch (Exception e) {
